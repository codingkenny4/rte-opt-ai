...
import { PlaceSuggestion, searchPlaceSuggestions } from '@/services/tmapService';
      ...
      {suggestions.length > 0 && (
        <View className="mt-2 rounded-xl border border-slate-800 bg-slate-950 overflow-hidden">
          {suggestions.map((suggestion) => (
            ...
            >
              <Text className="text-white text-sm font-semibold">{suggestion.name}</Text>
              <Text className="text-slate-400 text-xs mt-1">{suggestion.address}</Text>
            </TouchableOpacity>
          ))}
        </View>
      )}
